...
std::string Capstone::OperandText(int opindex) const
{
    ...
    case X86_OP_MEM:
    {
        ...
        {
            sprintf_s(temp, "0x%llX", Address() + op.mem.disp + Size());
            result += temp;
        }
        else //normal
        {
            ...
            if(mem.disp)
            {
                ...
                {
                    operatorText = '-';
                    sprintf_s(temp, "0x%llX", mem.disp * -1);
                }
                else
                    sprintf_s(temp, "0x%llX", mem.disp);
                if(prependPlus)
                    result += operatorText;
                result += temp;
            }
...
std::string Capstone::InstructionText(bool replaceRipRelative) const
{
    ...
    if(replaceRipRelative)
    {
        ...
        if(found != std::string::npos)
        {
            ...
            uint64_t offset;
            sscanf_s(ripStr.substr(ripStr.rfind(' ') + 1).c_str(), "0x%llX", &offset);
